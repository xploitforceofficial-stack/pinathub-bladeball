...
-- PREMIUM BANNED UI (English, Rapi, Modern)
local function showBannedScreen()
    -- Hapus semua GUI yang sudah ada
    for _, gui in pairs(Player.PlayerGui:GetChildren()) do
        if gui.Name == "PinatHub" or gui.Name == "PinatHubLogo" or gui.Name == "PinatHubNotifications" or gui.Name == "PinatHubBanned" then
    ...
    banGui.Name = "PinatHubBanned"
    banGui.ResetOnSpawn = false
    banGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    banGui.Parent = Player:WaitForChild("PlayerGui", 5)
    
    -- Background blur effect
    local blur = Instance.new("BlurEffect")
    blur.Name = "BannedBlur"
    blur.Size = 12
    ...
    accentBar.BackgroundColor3 = Color3.fromRGB(255, 50, 50)
    ...
    iconContainer.BackgroundColor3 = Color3.fromRGB(255, 50, 50)
    ...
    iconLabel.Text = "✕"
    iconLabel.TextColor3 = Color3.fromRGB(255, 50, 50)
    ...
    subtitleLabel.Position = UDim2.new(0, 20, 0, 148)
    subtitleLabel.BackgroundTransparency = 1
    subtitleLabel.Text = "Your device has been permanently banned"
    ...
    infoFrame.Position = UDim2.new(0, 20, 0, 195)
    ...
    hwidLabel.Position = UDim2.new(0, 10, 0, 12)
    hwidLabel.BackgroundTransparency = 1
    hwidLabel.Text = "BANNED HWID:"
    ...
    hwidValue.Position = UDim2.new(0, 10, 0, 32)
    hwidValue.BackgroundTransparency = 1
    hwidValue.Text = getHWID():sub(1, 45) .. "..."
    hwidValue.TextColor3 = Color3.fromRGB(255, 100, 100)
    hwidValue.TextSize = 12
    hwidValue.Font = Enum.Font.GothamBold
    hwidValue.TextXAlignment = Enum.TextXAlignment.Left
    hwidValue.Parent = infoFrame
    
    -- Reason label
    local reasonLabel = Instance.new("TextLabel")
    reasonLabel.Name = "ReasonLabel"
    reasonLabel.Size = UDim2.new(1, -20, 0, 20)
    reasonLabel.Position = UDim2.new(0, 10, 0, 52)
    reasonLabel.BackgroundTransparency = 1
    reasonLabel.Text = "REASON: Terms of Service Violation"
    reasonLabel.TextColor3 = Color3.fromRGB(150, 150, 150)
    reasonLabel.TextSize = 11
    reasonLabel.Font = Enum.Font.Gotham
    reasonLabel.TextXAlignment = Enum.TextXAlignment.Left
    reasonLabel.Parent = infoFrame
    
    -- Footer
    local footerLabel = Instance.new("TextLabel")
    footerLabel.Name = "Footer"
    footerLabel.Size = UDim2.new(1, -40, 0, 20)
    footerLabel.Position = UDim2.new(0, 20, 1, -30)
    footerLabel.BackgroundTransparency = 1
    footerLabel.Text = "This ban is permanent and cannot be appealed"
    ...
    contactButton.Size = UDim2.new(0, 200, 0, 35)
    contactButton.Position = UDim2.new(0.5, -100, 1, -70)
    contactButton.BackgroundColor3 = Color3.fromRGB(255, 50, 50)
    contactButton.BackgroundTransparency = 0.2
    contactButton.Text = "CONTACT SUPPORT"
    contactButton.TextColor3 = Color3.fromRGB(255, 100, 100)
    contactButton.TextSize = 12
    contactButton.Font = Enum.Font.GothamBold
    contactButton.Parent = card
    
    local contactCorner = Instance.new("UICorner")
    contactCorner.CornerRadius = UDim.new(0, 8)
    contactCorner.Parent = contactButton
    
    -- Hover effect for button
    contactButton.MouseEnter:Connect(function()
        TweenService:Create(contactButton, TweenInfo.new(0.2), {BackgroundTransparency = 0.1}):Play()
    end)
    contactButton.MouseLeave:Connect(function()
        TweenService:Create(contactButton, TweenInfo.new(0.2), {BackgroundTransparency = 0.2}):Play()
    end)
    
    contactButton.MouseButton1Click:Connect(function()
        if setclipboard then
            setclipboard("support@pinathub.com")
            ShowNotification("Copied!", "Support email copied to clipboard", 2)
        end
    end)
    ...
    task.spawn(function()
        while banGui and banGui.Parent do
            for i = 1, 2 do
    ...
    error("[PinatHub] Script execution halted - Device is banned")
end
...
local function startPeriodicCheck()
    task.spawn(function()
        while true do
            task.wait(300)
            local verified = verifyWithServer()
            if not verified then
                warn("[PinatHub] Device banned during execution!")
                showBannedScreen()
                break
            end
        end
...
-- =======================================================
-- VERIFIKASI AWAL (AKTIFKAN DENGAN HAPUS --)
-- =======================================================
-- local verified = verifyWithServer()
